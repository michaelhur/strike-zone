...
const getGames = async () => {
    ...
    if (gameCount) {
        ...
        dates.forEach((d) => {
            const games = d.games;
            idList = [...games.filter((game) => game.status.statusCode === 'F').map((game) => game.gamePk), ...idList];
        });
    }

    if (idList.length) {
        for await (const id of idList) {
            const response = await getAtBat(id);
...
const getAtBat = async (gameId = 717898) => {
    try {
        ...
        for await (const play of playData) {
            ...
            if (eventType === 'atBat') {
                ...
                const atBat: UpsertAtBat = {
                    id: `${officialDate}_${gameId}_${atBatIndex}`,
                    gameId,
                    atBatIndex,
                    isTopInning,
                    inning,
        ...
        console.log(`${gameId} succeed`);
        return res;
    } catch (e) {
        console.log(`${gameId} error with ${e}`);
    }
};
